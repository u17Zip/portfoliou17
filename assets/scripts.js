...
// Leave owner and repo empty when the site is hosted on username.github.io/repository.
// If the repository is not detected automatically, enter values here.
...
async function getFolderFiles(path) {
  ...
  for (const entry of entries) {
    if (entry.type === 'file' && getFileType(entry.name)) files.push({ name: entry.name, path: entry.path, type: getFileType(entry.name), url: entry.download_url || rawUrl(repository, entry.path) });
    if (entry.type === 'dir') files.push(...await getFolderFiles(entry.path));
  }
  return files;
}
...
function setLoading(message = 'Загрузка файлов...') {
  $('#projectGrid').innerHTML = `<p class="empty-state media-loading">${message}</p>`;
...
function renderMedia() {
  ...
  grid.innerHTML = '';
  if (!visible.length) { grid.innerHTML = '<p class="empty-state">В этой папке пока нет подходящих файлов.</p>'; return; }
  visible.forEach((item) => {
    ...
    if (item.type === 'image') {
      const image = document.createElement('img'); image.src = item.url; image.alt = item.name; image.loading = 'lazy'; image.decoding = 'async'; frame.appendChild(image); frame.addEventListener('click', () => openLightbox(item));
    } else {
      const video = document.createElement('video'); video.src = item.url; video.controls = true; video.preload = 'metadata'; video.playsInline = true; frame.appendChild(video);
    }
    const openButton = document.createElement('button'); openButton.className = 'media-open'; openButton.type = 'button'; openButton.title = 'Открыть'; openButton.textContent = '↗'; openButton.addEventListener('click', () => openLightbox(item)); frame.appendChild(openButton);
    const footer = document.createElement('div'); footer.className = 'media-footer';
    const fileName = document.createElement('span'); fileName.className = 'media-file-name'; fileName.textContent = item.name;
    const fileType = document.createElement('span'); fileType.className = 'media-file-type'; fileType.textContent = item.type === 'image' ? 'IMAGE' : 'VIDEO';
    footer.append(fileName, fileType); card.append(frame, footer); grid.appendChild(card);
  });
}
async function openFolder(folder) {
  state.folder = folder; const info = folderInfo[folder];
  ...
  $('.folders-grid').hidden = true; $('.project-toolbar').hidden = true; $('.project-results').hidden = false; setLoading();
  try { state.media = shuffle(await getFolderFiles(`projects/${folder}`)); renderMedia(); }
  catch (error) {
    console.error(error);
    ...
    $('#projectGrid').innerHTML = `<p class="empty-state">${message}</p>`; $('#projectCount').textContent = 'ERROR';
  }
}
function closeFolder() { state.folder = null; state.media = []; $('.folders-grid').hidden = false; $('.project-toolbar').hidden = false; $('.project-results').hidden = true; }
function setupFilters() {
  const filters = $$('.filter'); const definitions = [['all', state.language === 'en' ? 'All' : 'Все'], ['image', state.language === 'en' ? 'Images' : 'Фото'], ['video', state.language === 'en' ? 'Videos' : 'Видео']];
  filters.forEach((button, index) => { if (!definitions[index]) return; button.dataset.filter = definitions[index][0]; button.textContent = definitions[index][1]; button.onclick = () => { filters.forEach((item) => item.classList.remove('active')); button.classList.add('active'); state.filter = button.dataset.filter; if (state.folder) renderMedia(); }; });
}
function setLanguage() {
  state.language = state.language === 'ru' ? 'en' : 'ru'; document.documentElement.lang = state.language; $('#langToggle').textContent = state.language.toUpperCase();
  $$('[data-ru][data-en]').forEach((node) => { node.textContent = node.dataset[state.language]; }); setupFilters();
  if (state.folder) { const info = folderInfo[state.folder]; $('#folderTitle').textContent = state.language === 'en' ? info.enTitle : info.title; $('#folderDescription').textContent = state.language === 'en' ? info.enDescription : info.description; }
  showSection($('.nav-item.active')?.dataset.section || 'about');
}
function ensureLightbox() {
  let lightbox = $('#mediaLightbox'); if (lightbox) return lightbox;
  lightbox = document.createElement('div'); lightbox.id = 'mediaLightbox'; lightbox.className = 'media-lightbox'; lightbox.innerHTML = '<div class="lightbox-backdrop"></div><button class="lightbox-close" type="button" aria-label="Закрыть">×</button><div class="lightbox-content"></div>'; document.body.appendChild(lightbox);
  ...
  lightbox.querySelector('.lightbox-backdrop').addEventListener('click', close); lightbox.querySelector('.lightbox-close').addEventListener('click', close); document.addEventListener('keydown', (event) => { if (event.key === 'Escape') close(); }); return lightbox;
}
function openLightbox(item) {
  const lightbox = ensureLightbox(); const content = lightbox.querySelector('.lightbox-content'); content.replaceChildren();
